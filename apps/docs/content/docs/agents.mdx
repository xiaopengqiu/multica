...
New agents default to **Workspace**, so future teammates are available to everyone in the current workspace across the Agents page, issue and project selectors, Chat, `@` mentions, and delegation. Agents never appear in another workspace. Choose **Private** explicitly for a sensitive personal agent. For the full role-permission matrix, see [Members and roles](/members-roles).

<Callout type="info">
**Private agents are hidden from normal teammates.** Only the creator and workspace owners/admins can see or use them. For workspace agents, teammates can use the agent but cannot see its `custom_env` values or MCP config, and only the creator or a workspace owner/admin can edit those sensitive settings.
</Callout>

The Agents page opens to **All** for workspaces without a saved preference. Your agents carry a **You** label; teammate agents show their creator, so shared availability does not erase ownership.
