...
New agents default to `workspace`. This default applies to the UI, API, templates, and `multica agent create`; pass `private` explicitly when the agent contains sensitive personal instructions or should not be available to normal teammates.

Private agents are hidden from normal teammates. Workspace owners/admins retain administrative access. Workspace agents are visible and usable only inside their own workspace; sharing never crosses workspace boundaries.

Agent visibility is separate from **runtime visibility**. A private runtime limits who can create an agent on it or move an agent to it. Once its owner creates a workspace-visible agent, every workspace member can dispatch that agent without gaining permission to reconfigure the runtime. Regardless of agent visibility, `custom_env` values and MCP config remain visible/editable only to the agent creator or a workspace owner/admin.
