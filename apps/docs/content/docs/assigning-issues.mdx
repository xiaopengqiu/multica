...
- **Workspace agents** can be assigned by any member in that workspace, even when the agent runs on its creator's private runtime. **Private agents** are hidden from normal teammates and can only be assigned by their creator or a workspace owner/admin.
